...
gmodel = obj_from_hdf5file(os.path.join(cfg['lemon_glm_data'], 'lemon-group_glm-data.hdf5'), 'model')
design = obj_from_hdf5file(os.path.join(cfg['lemon_glm_data'], 'lemon-group_glm-data.hdf5'), 'design')
clean_data = obj_from_hdf5file(os.path.join(cfg['lemon_glm_data'], 'lemon-group_glm-data.hdf5'), 'data')
...
def plot_perm_column(clean_data, gmodel, name, ax1, ax2, subpanel=None):
    ...
    if p_name == 'Mean_OpenClosed':
        ...
        if np.any([p_name.find(tag) > -1 for tag in ['OpenClosed', 'Linear','Sex','Brain','Grey']]):
            scales = [-1, 1]
...
for ii in range(5):
    ...
    plot_perm_column(clean_data, gmodel, to_plot[ii], ax1, ax2)
