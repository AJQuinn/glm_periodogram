...
from lemon_support import (lemon_set_channel_montage,
                           lemon_create_heog,
...
if __name__ == '__main__':
    ...
    if subj == 'all':
        ...
        goods = osl.preprocessing.run_proc_batch(config, st.match_files, proc_outdir, overwrite=True, extra_funcs=extra_funcs, dask_client=True)
    else:
        # Run a single subject
        inputs = st.get(subj=subj)
        if len(inputs) == 0:
            print(f"Subject '{subj}' not found in raw_data directory")
        osl.preprocessing.run_proc_chain(inputs[0], config, outdir=proc_outdir, extra_funcs=extra_funcs)